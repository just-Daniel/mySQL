...
app.post('/articles', (request, response) => {
  ...
  if (data.title && data.body) {
    ...
    if (data.id) {
      db.query(`UPDATE articles SET title = ?, body = ?, updated_date = ? 
      WHERE id = ? AND user_id = ?`,
      [data.title, data.body, new Date(), data.id, data.user_id],
      (err, rows, fields) => {
        console.log(data.id);
        if (err) {
      ...
      db.query(`INSERT INTO articles VALUES(?, ?, ?, ?, ?, ?)`,
          [null, data.title, data.body, new Date(), new Date, data.user_id],
          (err, rows, fields) => {
            if (err) {
              response
                  .status(400)
